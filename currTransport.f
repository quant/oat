program Sheglov
  ...
  real(WP) :: sum,sled, gtot,Jmax,jx,jy
  complex(WP) :: emt, emkt
  real(WP) :: tv,dx, y0, x0, W_y, Umin, Umax, Umean, Utemp
  complex(WP), parameter::zzero=(0.0,0.0)
  complex(WP), parameter::zone = (1.0,0.0)
  integer isum, ixshift, iyshift
!      isim = 1
     goto 12
     vxy%xmax = 1250!2500!2*2*800.
     vxy%ymax = 2000!4000!4*2*800.
     vxy%xmin = 0
     vxy%ymin = 0
     vxy%nx   = 501!321!641!161!81
     vxy%hx   = 2.5!10
     vxy%ny   = 801!641!641!161!81
     vxy%hy   = 2.5!10
     x0 = 0.5*vxy%xmax
     y0 = 0.5*vxy%ymax
     dx = 120!60!200!0.25*x0
     ...
     do i = 1, vxy%nx
        ...
        do j = 1, vxy%ny
           y = vxy%hy*(j-1)
           vxy%u(i,j) = W_y*exp(-((x-x0)/dx)**2)*(y-y0)**2
!           vxy%u(i,j) = 5.*exp(-0.5*((x-x0)/dx)**2)+25.*(y/y0-1.)**8
     ...
     write(*,*) Umin, Umax
     goto 11
12     call p2d_read_n(vxy,'U_dis5nmqds_h5nm.dat')!disorder case
!12     call p2d_read_n(vxy,'U_idqds_5nm.dat')!ideal case
     goto 11
     iyshift = 0
     ixshift = 20
   ...
   write(*,*) vxy%nx, vxy%ny, vxy%hx, vxy%hy
!11   call p2d_write_n(vxy,'Uxytest.dat')
!  stop
11  open(DTOUT1,file=DATFILE,status='replace',iostat=ierr)!GatE_B
