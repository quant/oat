...
#if defined(WANT_1NORM_RCOND) && WANT_1NORM_RCOND
    double anorm_1 = 0;
    for (int c = 0; c < nw; ++c)
    {
        double colnorm = 0;
...
//--------------Joule heat Fraction----------------
       {
        ...
        for (int i = 0; i < ni; ++i)
        {
            double q544 = fabs(this->IdifV[i]);
//            if(q > IdVmax * 0.1) nt++;
            if(q544 > IdVmax/ImaxV * 0.1) nt++;
        }
...
*/
    std::sort(res.begin(),res.end(),MyLessThan(W));
...
*/
    ...
    std::sort(res.begin(),res.end(),MyGreaterThan(I));
//  qSort(res.begin(),res.end(),MyLessThan(I));

    return res;
}
MYVECTOR<int> Percol2D::index_for_sorted_difV() const
{
    MYVECTOR<int> res(nI());
    for (int i=0; i<res.size(); ++i)
    {
...
*/
    std::sort(res.begin(),res.end(),MyGreaterThan(difV));
    return res;
}
MYVECTOR<int> Percol2D::index_for_sorted_IdifV() const
{
    MYVECTOR<int> res(nI());
    for (int i=0; i<res.size(); ++i)
    {
...
*/
    ...
    std::sort(res.begin(),res.end(),MyGreaterThan(IdifV));
    return res;
}
MYVECTOR<int> Percol2D::index_for_sorted_Sigma() const
{
    MYVECTOR<int> res(nI());
    for (int i=0; i<res.size(); ++i)
    {
...
*/
    ...
    std::sort(res.begin(),res.end(),MyGreaterThan(Sigma));
