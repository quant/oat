program Sheglov
  ...
  real(WP), allocatable :: CuryN(:,:), CurxN(:,:), CuryNold(:,:), CurxNold(:,:)
  ...
  integer isum, ixshift, iyshift
      isim = 1
!     goto 12
     vxy%xmax = 2*2*800!2*2*800.
     vxy%ymax = 2*2*800!2*2*800.
     vxy%xmin = 0
     vxy%ymin = 0
     vxy%nx   = 321!641!161!81
     vxy%hx   = 10
     vxy%ny   = 321!641!161!81
     vxy%hy   = 10
     x0 = 0.5*vxy%xmax
     y0 = 0.5*vxy%ymax
     dx = 200!0.25*x0
...
!
  ...
  allocate(curxNold(1:m,1:Nx))
  allocate(curyNold(1:m,1:Nx))
...
!  Gnn(n)GammaR(n)(Gnn(n))*(sigmaL(n))*
      ...
      CuryN = CuryN + CuryEQ
      CurxN = CurxN + CurxEQ
      if(isim.eq.1.and.Btesla.ne.0) then
            isim = 2
            goto 15
      end if
      if(Btesla.ne.0) then
      CurxN = 0.5*(CurxN + CurxNold)
      CuryN = 0.5*(CuryN + CuryNold)
      end if
    do n = 2, Nx-3
       do i = 1, M-1
